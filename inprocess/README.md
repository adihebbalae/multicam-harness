...
| `SegmentSelectMethod` | `harnesses.segment_select` | Each clip split into 8 equal-time segments (or, under `--segment-seconds T`, consecutive T-second segments); the top-K segments per clip kept by relevance (the `viclip` scorer embeds each segment jointly as one 8-frame tube, per answer option under `_opt`; `_auto` queries the event statements, else the informative options, else the question; `--segments-keep 0` derives K from the frame budget and stream count; `--seg-select global` takes that top-K over every clip at once, `--seg-floor` reserving a minimum per clip first), their frames deduped question-wide and thinned evenly to the budget | 1 |
