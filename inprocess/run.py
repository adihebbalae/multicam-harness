...
# prechecks
# --------------------------------------------------------------------------- #
def scorer_precheck(scorers):
    """Resolve the transformers scorers before anything loads a model.

    The frame-level and query-search arms load their scorer lazily, at the first
    record, inside the per-record prepare step — which is caught into an error
    row. An absent or unresolvable checkpoint therefore costs a whole leg of
    error rows and still exits zero, after the backend has been paid for. Ask
    for the config up front instead, where a failure is one line and no GPU.
    """
    from transformers import AutoConfig
    for name in sorted(x for x in scorers if x and x != VICLIP_SCORER):
        try:
            AutoConfig.from_pretrained(name)
        except Exception as exc:
            raise SystemExit(
                f"the selection scorer '{name}' does not resolve "
                f"({type(exc).__name__}: {exc}). The arms load it lazily at the "
                "first record, so an absent one becomes a whole leg of error "
                "rows. Fetch it, or point the run at a cached checkpoint:\n"
...
def main():
    ...
    with open(args.subset, encoding="utf-8") as fh:
        data = json.load(fh)
    ...
    # A SIGKILL mid-write leaves a torn trailing line. iter_rows skips it, but
    # appending onto it would glue the next row into the fragment and lose a
    # generation that was already paid for, so close the line first.
    if os.path.exists(out) and os.path.getsize(out):
        with open(out, "rb") as probe:
            probe.seek(-1, os.SEEK_END)
            torn = probe.read(1) != b"\n"
        if torn:
            with open(out, "a", encoding="utf-8") as repair:
                repair.write("\n")
