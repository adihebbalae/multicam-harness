...
A SKIP IS NOT A PASS: the comparison also needs a subset whose videos are
actually on disk. ``main()`` tries, in order: ``CVBENCH_SUBSET`` +
``CVBENCH_VIDEO_ROOT`` if both are set; the CVBench pool under the fork's
CVBench video directory; then the surveillance pool that ships in
``data/subsets/`` under ``video_roots.crossview`` from ``configs/datasets.yaml``
(relative roots resolve inside the fork checkout). If none of them has three
usable records the run exits 0 with a SKIP naming every candidate it tried —
grep for the "pairs byte/pixel-identical" line, not the exit code, before
claiming equivalence.

Coverage is the originally ported arms only (the PAIRS list below); the
selection arms are not compared here, so a green run says nothing about them.

sys.path hygiene (IMPORTANT — the ordering below is load-bearing):
  * The fork's package is ``bench.*`` and this repo's port lives under
    ``inprocess.*`` (the top-level ``harnesses``/``models`` here belong to the
    original vLLM harness and are never imported by this test), so both trees
    can sit on sys.path without any import naming collision between them.
  * BUT importing ``bench.*`` triggers ``bench.reuse``, which does
    ``sys.path.insert(0, <fork>/Video-R1/src)`` — that directory jumps AHEAD of
    this repo for every later import. Video-R1/src carries top-level shims
    like ``eval_thinking``; to guarantee nothing from the fork ever shadows
    this repo's modules, this repo's path goes FIRST and ALL of its modules
    are imported BEFORE the fork path is added. (No actual collision was hit
    in this test, but the ordering keeps it that way.)
"""
import hashlib
import json
import os
...
from inprocess.models import clients as new_clients                      # noqa: E402
from inprocess.models.clients import Backend, GenOut                     # noqa: E402
from inprocess.harnesses.uniform import (                                # noqa: E402
    TemporalWeightedMethod as NewTemporal,
    CVBenchNativeMethod as NewNative,
)
from inprocess.harnesses.stitched import CentralizedMethod as NewCentralized      # noqa: E402
from inprocess.harnesses.decentralized import PerStreamMethod as NewPerStream     # noqa: E402
...
# mirrors inprocess.models.clients.GenOut exactly:
...
def video_frames_hash(path, nframes):
    ...
    inprocess.models.clients so the grid formula itself is the production
    one)."""
...
def pick_records(subset_path, video_root, n=3):
    ...
    out = []
    for rec in data:
        vids = [rec.get(f"video_{i}") for i in range(1, MAX_SLOTS + 1)]
...
def crossview_root():
    """``video_roots.crossview`` from configs/datasets.yaml, as the list of
    directories a relative path could mean (no YAML dependency: the file is a
    flat two-level mapping, so one regex is enough and the test keeps running in
    a bare env)."""
    cfg = os.path.join(NEW_REPO, "configs", "datasets.yaml")
    try:
        m = re.search(r"^\s*crossview:\s*(\S+)", open(cfg).read(), re.M)
    except OSError:
        return None
    if not m:
        return None
    ...
    if os.path.isabs(root):
        return [root]
    # a relative root is written against this repo's layout, but the release may
    # equally sit in the fork checkout — offer both, most local first
    return [os.path.join(NEW_REPO, root), os.path.join(FORK, root),
            os.path.join(FORK, root.split("/", 1)[-1])]


def candidates():
    """(label, subset, video_root) pairs to try, most specific first."""
    out = []
    sub, root = os.environ.get("CVBENCH_SUBSET"), os.environ.get("CVBENCH_VIDEO_ROOT")
    if sub and root:
        out.append(("env", sub, root))
    out.append(("cvbench",
                os.path.join(NEW_REPO, "data", "subsets", "cvbench_full_k4_subset.json"),
                os.path.join(FORK, "Video-R1", "src", "r1-v", "Evaluation", "CVBench")))
    sub = os.path.join(NEW_REPO, "data", "subsets", "crossview_meva1033_subset.json")
    for i, root in enumerate(crossview_root() or []):
        out.append((f"crossview[{i}]", sub, root))
    return out


def main():
    tried, subset, video_root, records = [], None, None, []
    for label, sub, root in candidates():
        got = pick_records(sub, root, n=3) if os.path.exists(sub) else []
        tried.append(f"{label}: {len(got)}/3 usable records "
                     f"({os.path.basename(sub)} under {root})")
        if len(got) >= 3:
            subset, video_root, records = sub, root, got
            break
    if len(records) < 3:
        print("SKIP: no candidate dataset has 3 records with all videos on disk. "
              "A SKIP IS NOT A PASS. Tried:")
        for t in tried:
            print(f"  {t}")
        sys.exit(0)
    ids = [r["id"] for r in records]
    print(f"fork      = {FORK} (pkg bench.*)")
    print(f"new repo  = {NEW_REPO} (pkg inprocess.*)")
